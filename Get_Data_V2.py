...
Energy = pd.read_csv(os.path.join('Reference Files', 'EnergyAbilities_Crosswalk.csv'))

current_datetime = '2019-07-10T9:00:00Z'


def get_data(header, current_datetime, stepsize, days_to_cover, Attachments, Avatars, Battlerites,
             Champions, MapIDs, Mounts, Outfits, Poses, Regions, Titles, Energy):
    ...
    try:
        ...
        for query_number in range(100000):
            ...
            for o in range(5):
                ...
                for p in range(len(rounds)):
                    if rounds[p]['type'] == 'Structures.RoundEvent':
                        ...
                        if rounds[p]['dataObject']['type'] == 'ENERGY_ABILITY_USED':
                            ...
                            energy_row = Energy[Energy['Ability ID'] == rounds[p]['dataObject']['value']]
                            
                            #New oldur R not in gameplay_json
                            if len(energy_row) == 0:
                                
                                energy_type.append('R')
                                
                            else:
                            
                                energy_type.append(energy_row['Type'].values[0])
                                     
                            #Add ex ability info (need to get reference df from gameplay json)
                        if rounds[p]['dataObject']['type'] == 'ULTIMATE_USED':
                            ultimate_used_round.append(rounds[p]['dataObject']['round'])
                            ultimate_used_user.append(rounds[p]['dataObject']['userID'])
                        if rounds[p]['dataObject']['type'] == 'ALLY_DEATH_ENERGY_PICKUP':
                            death_energy_round.append(rounds[p]['dataObject']['round'])
                            death_energy_user.append(rounds[p]['dataObject']['userID'])
                    ...
                    #Get start of round time
                    if rounds[p]['type'] == 'Structures.BattleritePickEvent':
                        start_game_time = rounds[p]['dataObject']['time']
                        
                    if rounds[p]['type'] == 'Structures.DeathEvent':
                        death_time.append(rounds[p]['dataObject']['time'])
                        death_user.append(rounds[p]['dataObject']['userID'])
                ...
                #Find number of ultimates used:
                Ultimates = pd.DataFrame(list(zip(ultimate_used_round, ultimate_used_user)), 
                                        columns = ['Round', 'userID'])
                Ultimates['Ultimates'] = 1
                Ultimates = Ultimates.groupby(['userID', 'Round']).sum().reset_index()
                
                #Number of ally death orb pick ups
                DeathOrbs = pd.DataFrame(list(zip(death_energy_round, death_energy_user)),
                                        columns = ['Round', 'userID'])
                DeathOrbs['DeathOrbs'] = 1
                DeathOrbs = DeathOrbs.groupby(['userID', 'Round']).sum().reset_index()
                
                #Find how many times each user used energy in every round
                EnergyUsed = pd.DataFrame(list(zip(energy_used_round, energy_used_user, energy_type)), 
                                          columns = ['Round', 'userID', 'Type'])
                
                RUsed = EnergyUsed[EnergyUsed['Type'] == 'R']
                EX1Used = EnergyUsed[EnergyUsed['Type'] == 'EX1']
                EX2Used = EnergyUsed[EnergyUsed['Type'] == 'EX2']
                
                RUsed = RUsed.groupby(['Round', 'userID']).count().reset_index()
                EX1Used = EX1Used.groupby(['Round', 'userID']).count().reset_index()
                EX2Used = EX2Used.groupby(['Round', 'userID']).count().reset_index()
                
                EnergyUsed = EnergyUsed.drop('Type', axis = 1)
                ...
                Ultimates['userID'] = Ultimates['userID'].astype(str)
                DeathOrbs['userID'] = DeathOrbs['userID'].astype(str)
                RUsed['userID'] = RUsed['userID'].astype(str)
                EX1Used['userID'] = EX1Used['userID'].astype(str)
                EX2Used['userID'] = EX2Used['userID'].astype(str)
                ...
                Game_AllRounds = pd.merge(Game_AllRounds, Ultimates, on = ['userID', 'Round'], how = 'left')
                Game_AllRounds = pd.merge(Game_AllRounds, DeathOrbs, on = ['userID', 'Round'], how = 'left')
                Game_AllRounds = pd.merge(Game_AllRounds, RUsed, on = ['userID', 'Round'], how = 'left')
                Game_AllRounds = Game_AllRounds.rename(columns = {'Type':'Num_R'})
                Game_AllRounds = pd.merge(Game_AllRounds, EX1Used, on = ['userID', 'Round'], how = 'left')
                Game_AllRounds = Game_AllRounds.rename(columns = {'Type':'Num_EX1'})
                Game_AllRounds = pd.merge(Game_AllRounds, EX2Used, on = ['userID', 'Round'], how = 'left')
                Game_AllRounds = Game_AllRounds.rename(columns = {'Type':'Num_EX2'})
                ...
                for index, row in Game_AllRounds.iterrows():
                    ...
                    if ((pd.isnull(row['Ultimates'])) and (pd.notnull(row['kills']))):
                        Game_AllRounds.loc[index, 'Ultimates'] = 0
                    
                    if ((pd.isnull(row['DeathOrbs'])) and (pd.notnull(row['kills']))):
                        Game_AllRounds.loc[index, 'DeathOrbs'] = 0
                        
                    if ((pd.isnull(row['Num_R'])) and (pd.notnull(row['kills']))):
                        Game_AllRounds.loc[index, 'Num_R'] = 0
                        
                    if ((pd.isnull(row['Num_EX1'])) and (pd.notnull(row['kills']))):
                        Game_AllRounds.loc[index, 'Num_EX1'] = 0
                        
                    if ((pd.isnull(row['Num_EX2'])) and (pd.notnull(row['kills']))):
                        Game_AllRounds.loc[index, 'Num_EX2'] = 0
                        
                #Get round/time end information
                round_num = []
                
                for i in range(len(round_finish)):
                    round_event = rounds[round_finish[i]]
                    round_finish_time.append(round_event['dataObject']['time'])
                    round_num.append(round_event['dataObject']['time'])
                    
                RoundEndTimes = pd.DataFrame(list(zip(round_num, round_finish_time)), columns = ['Round', 'Finish Time'])
                
                '''Above not used since no time API going down. Ideally, would be using Round end times and death times to
                #Find out when people die and when scenarios such as 1v2, 1v3, 2v3 come up and track if it was a win or loss
                #Also to track orb takes when situation is 1v1 to determine value of the orb to fix the energy_gain column
                #But since there is no time, energy_gain is not fixed'''
                ...
                for i in range(len(Game_AllRounds)):
                    ...
                    if total_num_players/2 >= 2:
                        ...
                        Ally1OrbKills.append(AllyDf['orb_kills'][0])
                    ...
                    if total_num_players/2 >= 3:
                        ...
                        Ally2OrbKills.append(AllyDf['orb_kills'][1])
                    ...
                    Enemy1OrbKills.append(EnemyDf['orb_kills'][0])
                    ...
                    if total_num_players/2 >= 2:
                        ...
                        Enemy2OrbKills.append(EnemyDf['orb_kills'][1])
                    ...
                    if total_num_players/2 >= 3:
                        ...
                        Enemy3OrbKills.append(EnemyDf['orb_kills'][2])
                ...
                Game_AllRounds['Ally1OrbKills'] = Ally1OrbKills
                ...
                Game_AllRounds['Ally2OrbKills'] = Ally2OrbKills
                ...
                Game_AllRounds['Enemy1OrbKills'] = Enemy1OrbKills
                ...
                Game_AllRounds['Enemy2OrbKills'] = Enemy2OrbKills
                ...
                Game_AllRounds['Enemy3OrbKills'] = Enemy3OrbKills
            ...
            if last_day != starting_day:
                ...
                for index, row in AllGames.iterrows():
                    ...
                    if row['serverType'] == 'QUICK2V2':
                        ...
                        #Orbs
                        team_orbs.append(row['orb_kills'] + row['Ally1OrbKills'])
                        enemy_orbs.append(row['Enemy1OrbKills'] + row['Enemy2OrbKills'])
                        ...
                        #Orbs
                        team_orbs.append(row['orb_kills'] + row['Ally1OrbKills'] + row['Ally2OrbKills'])
                        enemy_orbs.append(row['Enemy1OrbKills'] + row['Enemy2OrbKills'] + row['Enemy3OrbKills'])
                ...
                AllGames['Team Orbs'] = team_orbs
                AllGames['Enemy Orbs'] = enemy_orbs
                ...
                AllGames = AllGames.drop(['Enemy1HealthShards', 'Enemy2HealthShards', 'Enemy3HealthShards',
                                          ...
                                          'score_to_win', 'match_team', 'league_before', 'Ally1OrbKills',
                                          'Ally2OrbKills', 'Enemy1OrbKills', 'Enemy2OrbKills', 'Enemy3OrbKills'], axis = 1)
                ...
                FinalColNames = ['Game_ID', 'Patch_Version', 'Date', 'Match_Type', 'Ranking_Type', 'Server_Type', 'Season_ID', 'Region',
                                 ...
                                 'Total_Score', 'Highest_Score', 'Health_Shards', 'Energy_Shards', 'Ultimates', 'DeathOrbs', 'Num_R', 'Num_EX1', 'Num_EX2', 'Num_Energy_Used', 'First_Orb',
                                 ...
                                 'Team_Comp', 'Enemy_Comp', 'Team_Roles', 'Enemy_Roles', 'Team_Orbs', 'Enemy_Orbs', 'League']
                ...
                AllGames = AllGames[['Game_ID', 'Patch_Version', 'Date', 'Map', 'Match_Type', 'Ranking_Type', 'Server_Type', 'Solo_Queue', 'Season_ID', 
                                    'Region', 'City', 'Latitude', 'Longitude', 'Limited_MM',
                                    ...
                                    'Damage', 'Damage_Received', 'Protection', 'Protection_Received', 'Control', 'Control_Received',
                                    'Energy_Used', 'Energy_Gained', 'Abilities_Used', 'Ultimates', 'Num_R', 'Num_EX1', 'Num_EX2', 'Num_Energy_Used',
                                    'Orb_Kills', 'First_Orb', 'MVP', 'Round_Length', 'Round_Won',
                                    'Total_Score', 'Highest_Score', 'Health_Shards', 'Energy_Shards', 'DeathOrbs', 'Team_Orbs', 'Enemy_Orbs',
...
AllGames, LeaverDf, date, json_data, round_info, url = get_data(header, current_datetime, 9000, 2, Attachments,
